...
def Binomial_Tree(S_t, r, q, sigma, t1, T2, S_max_t, n, option_type="European"):
    # 1. 計算u, d, p
    delta_t = (T2 - t1) / n
    ...
    for period in range(n + 1):  # 第幾期
        period_max_list = []
        if period == 0:  # 第0期的S_max == max(S_max_t, S_t)
            period_max_list.append([max(S_max_t, S_t)])
        else:  # 第1期之後
            for node in range(period + 1):  # 第幾個node
                node_max_list = []
                if node == 0 or node == period:  # 最上面、最下面node的S_max == max(S_max_t, S_t, S_t_list[period]))
                    node_max_list.append(max(S_max_t, S_t, S_t_list[period][node]))  # 只有可能是那個node的股價，或S_max_t
                else:
                    for i in range(1, -1, -1):
                        for S_t_max_price in S_max_list[period - 1][node - i]:
                            if S_t_max_price >= S_t_list[period][node] and S_t_max_price not in node_max_list:
                                node_max_list.append(S_t_max_price)
                            elif S_t_max_price < S_t_list[period][node] and S_t_list[period][node] not in node_max_list:
                                node_max_list.append(S_t_list[period][node])   
