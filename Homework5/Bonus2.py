                ...
                if A[i + 1][j + 1][0] == A[i + 1][j + 1][M]:
                    ...
                    while True:
                        if abs(A[i + 1][j + 1][0] - A_d) <= 10 ** -8:
                            k_d = 0
                            break
                        elif A[i + 1][j + 1][k_d - 1] >= A_d and A_d >= A[i + 1][j + 1][k_d]:
                            break
...
def Binomial_Tree_Interpolation(S_t, K, r, q, sigma, t, T_minus_t, M, n, S_ave_t):
    delta_t = T_minus_t / n
    T = T_minus_t + t
    u = np.exp(sigma * (delta_t ** 0.5))
    d = 1 / u
        ...
        for j in range(i + 1):
            for k in range(M + 1):
                ...
                if A[i + 1][j][0] == A[i + 1][j][M]:  # 最上面或最下面
                    ...
                    k_u = math.ceil((A[i + 1][j][0] - A_u) / (A[i + 1][j][0] - A[i + 1][j][-1]) * M)
                    w_u = (A[i + 1][j][k_u - 1] - A_u) / (A[i + 1][j][k_u - 1] - A[i + 1][j][k_u])
                    eur_C_u = w_u * eur_C[i + 1][j][k_u] + (1 - w_u) * eur_C[i + 1][j][k_u - 1]
                    ame_C_u = w_u * ame_C[i + 1][j][k_u] + (1 - w_u) * ame_C[i + 1][j][k_u - 1]
                ...
                if A[i + 1][j + 1][0] == A[i + 1][j + 1][M]:
                    ...
                    k_d = math.ceil((A[i + 1][j + 1][0] - A_d) / (A[i + 1][j + 1][0] - A[i + 1][j + 1][-1]) * M)
                    w_d = (A[i + 1][j + 1][k_d - 1] - A_d) / (A[i + 1][j + 1][k_d - 1] - A[i + 1][j + 1][k_d])
                    eur_C_d = w_d * eur_C[i + 1][j + 1][k_d] + (1 - w_d) * eur_C[i + 1][j + 1][k_d - 1]
                    ame_C_d = w_d * ame_C[i + 1][j + 1][k_d] + (1 - w_d) * ame_C[i + 1][j + 1][k_d - 1]
                eur_C[i][j][k] = (p * eur_C_u + (1 - p) * eur_C_d) * np.exp(-r * delta_t)
...
def get_result(S_t, K, r, q, sigma, t, T_minus_t, M, n, S_ave_t):
    t1 = time.time()
    eur_binomial, ame_binomial = Binomial_Tree(S_t, K, r, q, sigma, t, T_minus_t, M, n, S_ave_t)
    t2 = time.time()
    print("1. Binomial Tree           Spend Time = {:.6f}".format(t2 - t1))
    ...
    t4 = time.time()
    print("2. Binomial Binary         Spend Time = {:.6f}".format(t4 - t3))
    ...
    t6 = time.time()
    print("3. Binomial Interpolation  Spend Time = {:.6f}".format(t6 - t5))
...
if __name__ == '__main__':
    ...
    print("t = 0")
    get_result(S_t, K, r, q, sigma, 0, T_minus_t, M, n, S_ave_t)
    print("t = 0.25")
    get_result(S_t, K, r, q, sigma, 0.25, T_minus_t, M, n, S_ave_t)
